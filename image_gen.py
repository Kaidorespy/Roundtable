...
class ImageGenerator:
    ...
    def generate_avatar(
        ...
        """
        dbg.image(f"▶ generate_avatar: {partner_name or partner_id} x{count}")

        if not self.is_available():
            dbg.image("✗ ComfyUI not running")
            raise RuntimeError("ComfyUI not running")

        # Use preset defaults if not specified
        width = width or self.default_width
        height = height or self.default_height
        dbg.image(f"  ├─ Size: {width}x{height}, preset: {self.model_preset}")
        ...
        for i in range(count):
            ...
            with _comfy_lock:
                ...
                for j, img_data in enumerate(images):
                    ...
                    # Add LoRA names (if any enabled) - prefer display_name for readability
                    lora_names = [sanitize(l.get('display_name') or l.get('name', ''), 40) for l in combined_loras if l.get('enabled', True) and (l.get('display_name') or l.get('name'))]
                    lora_part = f"_lora-{'-'.join(lora_names)}" if lora_names else ''
                    if combined_loras:
                        print(f"[image_gen] LoRAs for filename: {[(l.get('display_name'), l.get('name'), l.get('enabled')) for l in combined_loras]}")
                        print(f"[image_gen] LoRA part: '{lora_part}'")
                ...
                dbg.image(f"  ├─ Saved: {filepath.name}")

        dbg.image(f"✓ generate_avatar complete: {len(results)} images")
        return results
    ...
    def generate_scene(
        ...
        Args:
            captured_loras: Room LoRAs captured at queue time (overrides self.loras if provided)
        """
        dbg.image(f"▶ generate_scene: room={room_id}")
        ...
        dbg.image(f"  ├─ Size: {width}x{height} (landscape)")
        ...
        if images:
            # Build filename with generation metadata (similar to selfies)
            def sanitize(s, max_len=20):
                return re.sub(r'[^\w\-]', '', s.replace(' ', '_'))[:max_len] if s else None

            room_part = sanitize(room_id, 30) or 'scene'
            preset_part = f"_{self.model_preset}" if self.model_preset else ''
            sampler_short = (self.sampler or 'euler')[:6]
            settings_part = f"_{sampler_short}_{self.steps}s_{self.cfg}cfg"

            # Add LoRA names (if any enabled) - prefer display_name for readability
            lora_names = [sanitize(l.get('display_name') or l.get('name', ''), 40) for l in effective_loras if l.get('enabled', True) and (l.get('display_name') or l.get('name'))]
            lora_part = f"_lora-{'-'.join(lora_names)}" if lora_names else ''

            filename = f"scene_{room_part}{preset_part}{settings_part}{lora_part}_{seed}.png"
            filepath = self.scenes_dir / filename
            filepath.write_bytes(images[0])

            # Save prompt alongside image for gallery tooltips
            prompt_file = self.scenes_dir / f"scene_{room_part}{preset_part}{settings_part}{lora_part}_{seed}.txt"
            prompt_file.write_text(prompt, encoding='utf-8')
    ...
    def copy_scene_to_galleries(self, scene_path: Path, partner_ids: list[str]) -> list[Path]:
        """
        Copy a scene image to each participant's gallery folder.

        Args:
            scene_path: Path to the scene image
            partner_ids: List of partner IDs to copy to

        Returns:
            List of paths where scene was copied
        """
        if not scene_path.exists():
            print(f"[image_gen] copy_scene_to_galleries: Source not found: {scene_path}")
            return []
        ...
        prompt_path = scene_path.with_suffix('.txt')
        prompt_data = prompt_path.read_text(encoding='utf-8') if prompt_path.exists() else None

        for partner_id in partner_ids:
            try:
                partner_dir = self.get_partner_dir(partner_id)
                # Use same filename but prefix with "scene_" to distinguish
                dest_path = partner_dir / f"scene_{scene_path.name}"
                dest_path.write_bytes(scene_data)
                copies.append(dest_path)

                # Copy prompt too
                if prompt_data:
                    prompt_dest = dest_path.with_suffix('.txt')
                    prompt_dest.write_text(prompt_data, encoding='utf-8')

                print(f"[image_gen] Copied scene to {partner_id}'s gallery: {dest_path.name}")
            except Exception as e:
                print(f"[image_gen] Failed to copy scene to {partner_id}: {e}")
...
async def generate_scene_prompt(messages: list[dict], model: str = "deepseek-v3.1:671b-cloud") -> str:
    """

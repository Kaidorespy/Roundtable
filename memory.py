...
class MemoryConsolidator:
    ...
    def __init__(self, memory_store: MemoryStore, ollama_base_url: str = "http://localhost:11434", model: str = "deepseek-v3.1:671b-cloud"):
    ...
    async def consolidate(
        ...
        """
        dbg.memory(f"▶ Consolidation starting for {partner_name} in {room_id} (mode={memory_mode})")

        if memory_mode == "none":
            dbg.memory(f"  └─ Skipped: memory_mode=none")
            return Memory()

        memory = self.store.get_memory(partner_id, room_id, memory_mode)
        dbg.memory(f"  ├─ Existing anchors: {len(memory.anchors)}, resonance topics: {len(memory.resonance)}")
...
What themes or interests keep emerging? (not facts, but patterns)"""
        ...
        dbg.memory(f"✓ Consolidation complete for {partner_name}: {len(memory.anchors)} anchors, {len(memory.resonance)} themes")
        return memory
    ...
    def should_consolidate(self, partner_id: str, room_id: str, memory_mode: str) -> bool:
        ...
        if memory_mode == "none":
            return False
        memory = self.store.get_memory(partner_id, room_id, memory_mode)
        should = memory.turn_count >= self.CONSOLIDATION_INTERVAL
        if should:
            dbg.memory(f"● Consolidation triggered: {memory.turn_count} turns >= {self.CONSOLIDATION_INTERVAL}")
        return should

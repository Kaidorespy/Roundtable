...
class InventoryKeeper:
    ...
    async def analyze_message(self, message: str, speaker_id: str, speaker_name: str,
        ...
        """
        dbg.dm(f"▶ InventoryKeeper analyzing message from {speaker_name}")
...
"""
        ...
        try:
            ...
            if result.get("has_inventory_action"):
                dbg.dm(f"  └─ Found {len(result.get('actions', []))} inventory actions")
            return result
        except Exception as e:
            dbg.dm(f"✗ InventoryKeeper analysis error: {e}")
            return None
...
class Dramaturge:
    ...
    async def consult(self, genre: str, mood: str, story_beat: str,
        ...
        """
        dbg.dm(f"▶ Dramaturge consult: {genre}/{mood} - {story_beat[:50]}...")
